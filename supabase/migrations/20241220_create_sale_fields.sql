-- Criar tabela sale_fields
CREATE TABLE sale_fields (
...
-- Habilitar RLS
ALTER TABLE sale_fields ENABLE ROW LEVEL SECURITY;

-- Políticas RLS
CREATE POLICY "Users can view sale fields for their company" ON sale_fields
  FOR SELECT USING (
    company_id IN (
...
CREATE POLICY "Users can insert sale fields for their company" ON sale_fields
...
CREATE POLICY "Users can update sale fields for their company" ON sale_fields
  FOR UPDATE USING (
    company_id IN (
      SELECT id FROM companies 
      WHERE id = auth.jwt() ->> 'company_id'::text::uuid
...
CREATE POLICY "Users can delete sale fields for their company" ON sale_fields
  FOR DELETE USING (
    company_id IN (
      SELECT id FROM companies 
      WHERE id = auth.jwt() ->> 'company_id'::text::uuid
