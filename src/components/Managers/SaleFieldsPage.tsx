...
export function SaleFieldsPage() {
  const { selectedCompanyId } = useCompany();
  const { toast } = useToast();
  
  // Usar selectedCompanyId como effectiveCompanyId
  const effectiveCompanyId = selectedCompanyId;
  const [fields, setFields] = useState<SaleField[]>([]);
  const [fieldSearchQuery, setFieldSearchQuery] = useState('');
  const [selectedFieldType, setSelectedFieldType] = useState('');
  const [newFieldName, setNewFieldName] = useState('');
  const [previewField, setPreviewField] = useState<SaleField | null>(null);
  ...
  const handleSenderChange = (newSender: string) => {
    ...
    if (!newSender.startsWith('venda_')) {
      ...
      formattedSender = `venda_${cleanName}`;
    } else {
      // Se já tem o prefixo, apenas formatar a parte após "venda_"
      const afterPrefix = newSender.substring(6); // Remove "venda_"
      ...
      formattedSender = `venda_${cleanAfterPrefix}`;
    }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-6">
        <div>
          <h3 className="text-lg font-semibold mb-4">Campos da Venda</h3>
          <p className="text-sm text-muted-foreground mb-6">
            Gerencie os campos personalizados para vendas. Arraste e solte para reordenar.
          </p>
