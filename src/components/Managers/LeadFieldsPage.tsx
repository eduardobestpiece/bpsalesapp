...
export function LeadFieldsPage() {
  ...
  const [previewField, setPreviewField] = useState<LeadField | null>(null);
  ...
  const generateDefaultSender = (fieldName: string): string => {
    ...
    return `lead_${cleanName}`;
  };

  // Carregar configurações do campo selecionado
  const loadFieldConfig = (field: LeadField) => {
  ...
  const handleSenderChange = (newSender: string) => {
    ...
    if (!newSender.startsWith('lead_')) {
      ...
      formattedSender = `lead_${cleanName}`;
    } else {
      // Se já tem o prefixo, apenas formatar a parte após "lead_"
      const afterPrefix = newSender.substring(5); // Remove "lead_"
      ...
      formattedSender = `lead_${cleanAfterPrefix}`;
    }
  ...
  const saveFieldConfig = async () => {
    ...
    try {
      ...
      // Recarregar as configurações do campo para garantir que tudo está atualizado
      if (previewField) {
        const updatedField = {
          ...previewField,
          name: fieldName,
    ...
    try {
      ...
      const mappedFields = (data || []).map((field: any) => {
        ...
        return {
          ...
          sender: field.sender || generateDefaultSender(field.name),
          // Configurações monetárias
  ...
  const onDragEnd = async (result: any) => {
    ...
    try {
      ...
      for (const update of updates) {
        await supabase
          .from('lead_fields' as any)
          .update({ order: update.field_order })
          .eq('id', update.id);
      }
    } catch (error) {
  ...
  const renderFieldPreview = (field: LeadField) => {
    ...
    switch (field.type) {
      ...
      case 'url':
        const handleUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
          ...
          const formattedValue = validateAndFormatURL(value);
          setUrlValue(formattedValue);
        };
        
        const isUrlValid = urlValue.trim() === '' || validateURLFormat(urlValue);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-6">
        <div>
          <h3 className="text-lg font-semibold mb-4">Campos Lead</h3>
          <p className="text-sm text-muted-foreground mb-6">
            Gerencie os campos personalizados para leads. Arraste e solte para reordenar.
          </p>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="space-y-2" style={{ marginBottom: '16px' }}>
              {(!fieldPlaceholder || ['checkbox', 'money', 'time', 'datetime', 'date', 'slider', 'address', 'name', 'connection', 'document'].includes(previewField.type)) && (
                <Label 
