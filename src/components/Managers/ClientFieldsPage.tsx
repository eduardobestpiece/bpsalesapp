...
export function ClientFieldsPage() {
  ...
  const [previewField, setPreviewField] = useState<ClientField | null>(null);
  ...
  const loadFieldConfig = (field: ClientField) => {
    ...
    // Carregar configurações específicas para seleção com conexão
    console.log('Loading selection connection config:', field.selection_list);
    setFieldSelectionList(field.selection_list || '');
    setSelectedSelectionItem(null);
  ...
  const handleSenderChange = (newSender: string) => {
    ...
    if (!newSender.startsWith('cliente_')) {
      ...
      formattedSender = `cliente_${cleanName}`;
    } else {
      // Se já tem o prefixo, apenas formatar a parte após "cliente_"
      const afterPrefix = newSender.substring(8); // Remove "cliente_"
      ...
      formattedSender = `cliente_${cleanAfterPrefix}`;
    }
  ...
  const loadConnectionItems = async (listType: string) => {
    ...
    console.log('Loading connection items for list type:', listType, 'company:', effectiveCompanyId);
    ...
    try {
      ...
      console.log('Loaded connection items:', data);
    ...
    try {
      ...
      switch (listType) {
        ...
        case 'motivos-perda':
          ...
          break;
        default:
          console.log('Lista não reconhecida:', listType);
          return;
      }
  ...
  const renderFieldPreview = (field: ClientField) => {
    ...
    switch (field.type) {
      ...
      case 'select':
        // Verificar se usa conexão ou opções manuais
        if (fieldSelectionConnection && fieldSelectionList) {
          // Usar itens da lista selecionada
          console.log('Rendering selection with connection:', {
            fieldSelectionList,
            selectionConnectionItems: selectionConnectionItems.length,
            selectedSelectionItem
          });
          
          const filteredSelectionItems = selectionConnectionItems.filter(item => {
            const searchField = item.name || item.title || item.result_name;
            return searchField?.toLowerCase().includes(selectionSearchQuery.toLowerCase());
          });

          if (fieldMultiselect) {
            // Renderizar multiseleção com itens da conexão
            return (
              <Select 
                value={selectedSelectionItem?.id || ''} 
                onValueChange={(value) => {
                  const item = selectionConnectionItems.find(item => item.id === value);
                  setSelectedSelectionItem(item);
                }}
                open={isSelectionDropdownOpen}
                onOpenChange={setIsSelectionDropdownOpen}
              >
                <SelectTrigger style={{ ...fieldStyle, ...fontStyle, ['--active-bc' as any]: '#E50F5E', ['--focus-bw' as any]: '2px' }} className="h-12 text-base focus:ring-0 focus:ring-offset-0 focus-visible:ring-0 focus-visible:ring-offset-0 focus-border focus-border">
                ...
                <SelectContent className="bg-[#2A2A2A] border-white/20 text-white max-h-80">
                  ...
                  {filteredSelectionItems.length > 0 ? (
                    filteredSelectionItems.map((item, index) => (
                      <div key={item.id} className="flex items-center space-x-2 p-2 hover:bg-white/10 cursor-pointer">
                        <Checkbox
                          id={`preview-selection-${item.id}`}
                          className="data-[state=checked]:bg-[var(--brand-primary,#E50F5E)] data-[state=checked]:border-[var(--brand-primary,#E50F5E)]"
                        />
                        <Label 
                          htmlFor={`preview-selection-${item.id}`}
                    ...
                    <div className="p-2 text-sm text-muted-foreground">
                      {selectionConnectionItems.length === 0 ? 'Nenhum item disponível' : 'Nenhum item encontrado'}
                    </div>
                  )}
                </SelectContent>
              </Select>
            ...
            // Renderizar seleção simples com itens da conexão
            return (
              <Select 
                value={selectedSelectionItem?.id || ''} 
                onValueChange={(value) => {
              ...
              >
                <SelectTrigger style={{ ...fieldStyle, ...fontStyle, ['--active-bc' as any]: '#E50F5E', ['--focus-bw' as any]: '2px' }} className="h-12 text-base focus:ring-0 focus:ring-offset-0 focus-visible:ring-0 focus-visible:ring-offset-0 focus-border focus-border">
                  <SelectValue placeholder={selectedSelectionItem ? (selectedSelectionItem.name || selectedSelectionItem.title || selectedSelectionItem.result_name) : getPlaceholder()} />
                </SelectTrigger>
                <SelectContent className="bg-[#2A2A2A] border-white/20 text-white" style={{...fontStyle, ['--selBg' as any]: '#E50F5E', ['--selFg' as any]: '#FFFFFF', fontSize: '16px'}}>
                  {fieldSearchable && (
                    <div className="p-2 border-b border-white/20">
          ...
          // Usar opções manuais (lógica original)
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-6">
        <div>
          <h3 className="text-lg font-semibold mb-4">Campos do Cliente</h3>
          <p className="text-sm text-muted-foreground mb-6">
            Gerencie os campos personalizados para clientes. Arraste e solte para reordenar.
          </p>
