...
>(({ className, trackClassName, rangeClassName, thumbClassName, trackStyle, rangeStyle, thumbStyle, ...props }, ref) => (
	<SliderPrimitive.Root
		...
		{...props}
	>
		<SliderPrimitive.Track style={trackStyle} className={cn("relative h-2 w-full grow overflow-hidden rounded-full bg-secondary", trackClassName)}>
			<SliderPrimitive.Range style={rangeStyle} className={cn("absolute h-full bg-primary", rangeClassName)} />
		</SliderPrimitive.Track>
		<SliderPrimitive.Thumb style={thumbStyle} className={cn("block h-5 w-5 rounded-full border-2 border-primary bg-background ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50", thumbClassName)} />
	</SliderPrimitive.Root>
))
