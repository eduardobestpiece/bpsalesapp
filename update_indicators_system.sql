-- Script para atualizar o sistema de indicadores
...
-- 5. Adicionar campo para controlar se o indicador é diário (sempre true no novo sistema)
ALTER TABLE indicators 
ADD COLUMN IF NOT EXISTS is_daily BOOLEAN DEFAULT TRUE;

-- 6. Atualizar comentários da tabela indicators
...
-- 7. Criar função para facilitar consultas de conversão
...
-- 8. Criar view para facilitar consultas de indicadores com conversões
...
-- 9. Criar função para validar conversões
...
WHERE table_name = 'funnel_stages' 
  AND column_name IN ('conversion_enabled', 'conversion_funnel_id', 'conversion_stage_id')
ORDER BY column_name;

-- Verificar se o campo is_daily foi adicionado
SELECT 
  column_name, 
  data_type, 
  is_nullable 
...
WHERE routine_name IN ('get_conversion_stages', 'validate_conversion_setup')
